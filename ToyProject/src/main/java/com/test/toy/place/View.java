...
public class View extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ArrayList<String> hashtag = dao.getHashTagSeq(seq); // 
		dto.setHashtag(hashtag);
		
		// 우리가 읽으려는 파일 변수 
		File file = new File(req.getRealPath("asset/place") + "\\" + dto.getPic());
		System.out.println(req.getRealPath("asset/place")+ "\\" + dto.getPic() );
			...
			if(dto.getPic().toLowerCase().endsWith(".png")
				...
				try {                                           // 매개변수로 받아서 읽는다.
					...
					if(gps.containsTag(GpsDirectory.TAG_LATITUDE)
						&& gps.containsTag(GpsDirectory.TAG_LONGITUDE)	) { // 위도 정보있냐? 경도 정보 있냐?
